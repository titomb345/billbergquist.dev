...
interface UseWebSocketOptions<TClient, TServer> {
  url: string | null;
  pendingMessage: TClient | null;
  getJoinMessage?: () => TClient | null;
...
export function useWebSocket<TClient, TServer>({ url, pendingMessage, getJoinMessage, onPendingMessageSent, onMessage, onStatusChange, onReplaced }: UseWebSocketOptions<TClient, TServer>) {
  ...
  useEffect(() => {
    ...
    const urlRef: { current: string | null } = { current: url };
